...
def tune(cost_func, n, m, step, trials, effectors=np.array([t1, d2, d3, t4, t5, t6])):
    ...
    for i in range(trials):
        ...
        for ALPHA in np.arange(n, m, step):
            ...
            ax_axis = [0]
            ay_axis = [0]
            inverse_kinematics = find_inverse_kinematics(goal, effectors, 10000, cost=cost_func, ALPHA=ALPHA,
                                                         ay_axis=ay_axis, ax_axis=ax_axis,
                                                         original_configuration=original_configuration)
            if inverse_kinematics[1] < ACCEPTED_DISTANCE and ay_axis[-1] < best_alpha_score:
                best_alpha = ALPHA
                best_alpha_score = ay_axis[-1]
